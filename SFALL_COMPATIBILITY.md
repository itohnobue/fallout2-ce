...
| Skills | get/set_critter_skill_points<br>get/set_available_skill_points<br>set_skill_max<br>set_critter_skill_mod<br>set_base_skill_mod<br>mod_skill_points_per_level | partial | set_skill_max wired into skill.cc. mod_skill_points_per_level stored; consumed by characterEditorUpdateLevel() (character_editor.cc:5758). get_critter_skill_points, get_available_skill_points registered and implemented (sfall_opcodes.cc). set_critter_skill_mod (0x81C7) and set_base_skill_mod (0x81C8): registered, values stored but engine integration pending. |
...
| Perks and traits | set_perk_image<br>set_perk_*<br>set_pyromaniac_mod<br>apply_heaveho_fix<br>set_swiftlearner_mod<br>has/set_fake_perk<br>has/set_fake_trait<br>set_selectable_perk<br>set_perkbox_title<br>show/hide_real_perks<br>perk_add_mode<br>clear_selectable_perks<br>add/remove_trait<br>seq_perk_freq<br>set_perk_name<br>set_perk_desc | implemented: set_perk_freq (0x8247), set_perk_level (0x817A), set_perk_name (0x8189), set_perk_desc (0x818A), set_fake_trait (0x81BC), set_fake_perk (0x81BB), has_fake_perk (0x81C1), has_fake_trait (0x81C2), set_selectable_perk (0x81BD), set_perkbox_title (0x81BE), show/hide_real_perks (0x81BF/0x81C0), add_trait (metarule), remove_trait (metarule) | set_perk_freq is fully integrated (gPerkFrequencyOverride wired into character_editor.cc). set_perk_level override fully integrated via perkSetMinLevel (perk.cc:588). FastShotFix supported via ddraw.ini. add_trait/remove_trait store/retrieve data; full engine display integration pending. perk_add_mode (0x81C3), clear_selectable_perks (0x81C4), set_pyromaniac_mod (0x81CB), set_swiftlearner_mod (0x81CD), set_hp_per_level_mod (0x81CE): registered, storage-only; engine integration pending. set_perk_image, apply_heaveho_fix, seq_perk_freq not yet implemented. |
...
| KeyPress | `HOOK_KEYPRESS` | ✅ | Third hook arg is SDL_Keycode (SDL keysym, not Windows VK_ codes). Differs numerically from sfall VK_ for letters and function keys. |
