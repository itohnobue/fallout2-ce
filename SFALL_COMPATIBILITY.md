...
| `WorldMapSlots=21` (RPU gate: `gl_k_modini.ssl:14`, `!= 21` → `signal_end_game`) | ✅ | H-06: gSfallConfig default 21 (`src/sfall_config.cc:85-92`) flows through `op_get_ini_setting` fallback (`src/sfall_ini.cc:661-680`). |
...
| Worldmap content: city.txt (61 areas), worldmap.txt (20 tiles), maps.txt (173 maps) | ✅ | CE loads all dynamically (`src/worldmap.cc`: city.txt 3214, worldmap.txt 2080, maps.txt 3416). Pipboy automap now covers all 173 maps — `AUTOMAP_MAP_COUNT` raised to 173 (`src/automap.h:26`). |
...
4. ~~P2 — Raise pipboy automap capacity above 160 for RPU's 173 maps~~ ✅ **DONE (2026-08-17):** `AUTOMAP_MAP_COUNT` raised from 160 to 173 (`src/automap.h:26`); the derived on-disk header `dataSize` is now `5 + AUTOMAP_OFFSET_COUNT * sizeof(int)` = 2081 bytes (`src/automap.cc:1218`). RPU maps 160-172 (EPA sublevels, SF Sheng's, Slaver Camp, safehouses, etc.) now get pipboy automap entries; mods with more than 173 maps remain clamped/rejected by `automapMapIndexIsValid` (`src/automap.h:90-95`). Pinned by `tests/test_automap.cc` (static_asserts: AUTOMAP_MAP_COUNT == 173, dataSize == 2081, last valid index 172).
