...
| Graphics | graphics_funcs_available<br>force_graphics_refresh<br>get_screen_width<br>get_screen_height<br>set_palette | ✅ | get_screen_width, get_screen_height fully implemented. set_palette (0x81F2) registered as safe no-op (SDL2 rendering does not support palette overrides). graphics_funcs_available (0x8165), force_graphics_refresh (0x81B0) registered as safe no-ops (SDL2 renderer does not use palettes or DirectX-style refreshes, `src/sfall_opcodes.cc:6606-6611,6679-6683,6935-6964`). |
| Shaders | load_shader<br>free_shader<br>activate_shader<br>deactivate_shader<br>set/get_shader_* | ✅ | Registered as safe no-ops: all 11 shader opcodes (load_shader, free_shader, activate_shader, deactivate_shader, set_shader_int, set_shader_float, set_shader_vector, get_shader_version, set_shader_mode, get_shader_texture, set_shader_texture) are registered with handlers that log a debug warning and push safe defaults (0 or no-op). No actual shader support — SDL2 rendering does not use GPU shaders. `src/sfall_opcodes.cc:6615-6723,6935-6964`.
| Perks and traits | set_perk_image<br>set_perk_*<br>set_pyromaniac_mod<br>apply_heaveho_fix<br>set_swiftlearner_mod<br>has/set_fake_perk<br>has/set_fake_trait<br>set_selectable_perk<br>set_perkbox_title<br>show/hide_real_perks<br>perk_add_mode<br>clear_selectable_perks<br>add/remove_trait<br>set_perk_name<br>set_perk_desc<br>set_hp_per_level_mod | ✅ | set_perk_freq is fully integrated (gPerkFrequencyOverride wired into character_editor.cc). set_perk_level override fully integrated via perkSetMinLevel (perk.cc:588). set_pyromaniac_mod consumed at combat.cc:4858. set_swiftlearner_mod consumed at stat.cc:822. set_hp_per_level_mod consumed at stat.cc:859,912. set_critter_skill_mod and set_base_skill_mod consumed at skill.cc:252,267,270. perk_add_mode, clear_selectable_perks, hide_real_perks integrated via sfallGet* accessors at character_editor.cc. add_trait/remove_trait fully implemented (sfall_metarules.cc:2984) with 1-arg and 3-arg forms; display integrated via gAddedTraits at character_editor.cc:2164. set_perk_name, set_perk_desc stored in perk override arrays and consumed in perk.cc via sfallGetPerk*Override() accessors. set_perk_image fully implemented (sfall_opcodes.cc:3632-3643, consumed at perk.cc:626). apply_heaveho_fix registered as safe no-op (handled via engine config, sfall_opcodes.cc:4289-4292). seq_perk_freq is not present in CE (no sfall equivalent opcode exists in this engine). **Savegame persistence:** Opcode string state (`set_perk_name`, `set_perk_desc`, `set_perkbox_title`) IS serialized to savegames via the sfall global vars v2 string map (`stringVars`). String values persist across save/load cycles — mod scripts do NOT need to re-call string setters after loading. |
| Virtual file system | fs_create<br>fs_copy<br>fs_find<br>fs_read/write_*<br>fs_delete<br>fs_size<br>fs_pos<br>fs_seek<br>fs_resize | ✅ | All 18 fs_* opcodes are registered and implemented (see `sfall_opcodes.cc:2892-2927`). **Caveat (C-06):** `fs_copy(src, dest)` rejects identical source/destination paths with -1 (`src/sfall_opcodes.cc:2579-2589`) — sfall's FScopy is memory-only and accepts same-path copies. RPU's UPU scripts call `fs_copy(path, path)` for in-place FRM FPS patches; those features are inert on CE (see the RPU section below). |
...
| Hooks / Hook functions | init_hook<br>get_sfall_arg<br>get_sfall_args<br>get_sfall_arg_at<br>set_sfall_return<br>set_sfall_arg<br>register_hook<br>register_hook_proc<br>register_hook_proc_spec | ✅ | See below for implemented hooks. `init_hook` is deprecated and will not be implemented. **Hook registration order:** `register_hook_proc` inserts at the end of the hook list (executed first, lowest priority); `register_hook_proc_spec` inserts at the beginning (executed last, can override return values). Hook iteration is reverse (last index → first), so highest-index hooks execute first. See `sfall_script_hooks.cc:275-284`. |
| Arrays / Array functions | create_array<br>temp_array<br>fix_array<br>get/set_array<br>resize_array<br>free_array<br>scan_array<br>len_array<br>save/load_array<br>array_key<br>arrayexpr | ✅ | **Array sort modes:** CE supports the standard sfall sort modes (-6: sort by value ascending, -7: sort by value descending). Modes -8 (sort by value reverse) and -9 (shuffle by value) are **CE extensions** beyond the standard sfall range — mod scripts using these modes will silently fail when run on sfall (where these modes do not exist). |
...
| Rotators detection (`read_byte(0x410003)==0xF4` + `metarule_exist("rotators")`) | ✅ | `src/sfall_opcodes.cc:158-160`, `src/sfall_metarules.cc:1229-1232` |
| CE detection (`ce_enabled` = `opcode_exists(0x823B)==false`) | ✅ | 0x823B `modified_ini` intentionally not implemented |
| Startup gate (`get_ini_setting("ddraw.ini\|...")` for AllowUnsafeScripting, DisableHorrigan, UseFileSystemOverride, Fallout1Behavior) | ⚠️ | With CE's shipped `files/ddraw.ini` all four resolve to 0 → `gl_0.int` shows a warning, force-writes the keys to 1 via `set_ini_setting`, and returns to main menu (`signal_close_game`). One-time first-run restart. Add the four keys to ddraw.ini or adjust the config-bridge defaults to remove the friction. |
| FO1 hit chance (`set_fo1_hit_chance(true)`, gl_fo1mechanics.ssl:104-106) | ✅ | Metarule H-04 (`sfall_metarules.cc:2379`); consumed in to-hit at `combat.cc:4833-4839` together with `gFallout1Behavior` |
...
| Encounter handling (`encounter_detection(false)`, gl_fo1mechanics.ssl:101) | ✅ | Metarule → `wmSetEncounterDetection` |
| HOOK_ENCOUNTER (gl_worldmap.ssl:53-63 handler) | ✅ | **CE now uses the sfall arg0 encoding**: arg0=0 random / arg0=1 local-map-enter, arg2=isSpecial; forced encounters do not fire the hook (`src/sfall_script_hooks.cc:737-796`). Ret0 overrides map id, ret0=-1 cancels (event type 0). See the HOOK_ENCOUNTER row in the Hooks table above. |
| Reaction thresholds (`set_reaction_thresholds(25, 75)`, gl_fo1mechanics.ssl:103) | ✅ | Metarule + `reaction.cc:40-46`; FO1/FO2 defaults preserved on game reset |
| Rest hours/strings (`rest_option_msgs(320)`, `set_rest_option(REST_OPTION_MORNING, 6)`, gl_0_settings.ssl:120-121) | ✅ | CE metarules (`sfall_metarules.cc:2416,2427`); FO1 defaults 6:00 wake / base 321 |
...
| Party armor appearance / weapon restrictions (gl_partyarmor.ssl) | ✅ | HOOK_INVENWIELD/ADJUSTFID/INVENTORYMOVE/CANUSEWEAPON, `art_exists`, `get/set_object_data`, `real_dude_obj` |
...
12. **Docs — Update stale SFALL_COMPATIBILITY.md rows**: HOOK_ENCOUNTER (code now sfall-compatible: arg0=0/1, arg2=special, no forced-fire — the "arg0=2 / arg0=1-for-special" text is wrong) and HOOK_KEYPRESS (arg1=DIK, arg2=VK — not SDL_Keycode); add this Et Tu section.

## RPU (Fallout 2 Restoration Project) Compatibility

CE supports the [Fallout 2 Restoration Project, updated (RPU)](https://github.com/BGforgeNet/Fallout2_Restoration_Project) (requires sfall 4.5; CE reports 4.5.1). RPU's own scripting surface is small: 4 sfall hooks, ~25 opcodes/metarules, and sfall ddraw.ini config keys. All of it is implemented in CE. The table below is a requirement-by-requirement audit against RPU master `f7c10859` (2026-08-10).
...
| `ExtraSaveSlots=1` | ✅ | WIRED — 10 save pages (`src/loadsave.cc:259`, gExtraSaveSlots). |
...
| Worldmap content: city.txt (61 areas), worldmap.txt (21 tiles), maps.txt (173 maps) | ✅ | CE loads all dynamically (`src/worldmap.cc`). Pipboy automap clamps at 160 maps (see remaining work). |
...
| Data files: ai.txt, party.txt, quests.txt, karmavar.txt, endgame.txt, vault13.gam | ✅ | All read by CE (`src/combat_ai.cc:398`, `src/party_member.cc:136`, `src/pipboy.cc:2860`, `src/character_editor.cc:7664`, `src/endgame.cc:1074`). |
| Hero appearance opcodes `set_hero_style`/`set_hero_race` (`epai37.ssl`) | ✅ | Implemented (`src/sfall_opcodes.cc:3764,3773`), store `HApStyle`/`HAp_Race` globals RPU reads. |
...
7. **P3 — Bundle `partyslot.png` asset** for the party-equipment loot UI (code was ported upstream; the binary asset was not added — see s2-impl-incorporate report).
